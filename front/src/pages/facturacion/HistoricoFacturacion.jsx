...
import { ArrowLeft, Search, Loader2, Eye, Printer } from 'lucide-react';
import { ordenesAPI } from '../../services/api';
import DetalleFacturacion from './DetalleFacturacion';
import { formatDateShort } from '../../utils/dateUtils';

export default function HistoricoFacturacion({ onBack }) {
  ...
  const fetchOrdenes = async () => {
    try {
      setLoading(true);
      // Obtener órdenes en estado Finalizado
      const response = await ordenesAPI.getAll({ estado: 'Finalizado' });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Histórico de Órdenes Finalizadas</h1>
        <p className="text-gray-600 mt-1">
          Consulta todas las órdenes que han sido finalizadas
        </p>
      </div>
      ...
      {ordenesFiltradas.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-gray-500 text-lg">
            {searchTerm ? 'No se encontraron órdenes con ese criterio' : 'No hay órdenes finalizadas'}
          </p>
        </div>
      ) : (
